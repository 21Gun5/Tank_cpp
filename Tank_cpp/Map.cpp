...
//void CMap::CustomizeMap(PTANK ptank, PTANK penemytank)
//{
//	DrawBorder();		//地图边界
//	//提示信息
//	setColor(12, 0);
...
void CMap::DrawBarr()
{
	for (int x = 0; x < MAP_X_WALL; x++)
	{
		for (int y = 0; y < MAP_Y; y++)
		{
			if (m_nArrMap[x][y] == 石块障碍) GotoxyAndPrint(x, y, "■");
			if (m_nArrMap[x][y] == 土块障碍) GotoxyAndPrint(x, y, "※");
		}
	}
}
...
void CMap::DrawBorder()
{
	...
	for (int x = 0; x < MAP_X; x++)
	{
		for (int y = 0; y < MAP_Y; y++)
		{
			if (m_nArrMap[x][y] == 地图边界)
			{
				GotoxyAndPrint(x, y, "■");
			}
			if (m_nArrMap[x][y] == 我家泉水)
			{
				setColor(12, 0);
				GotoxyAndPrint(x, y, "★");
				setColor(7, 0);
