...
void CTank::ManipulateMyTank(CTank* pMyTank, CTank* pEnemyTank, CMap map, CGame& game)
{
	...
	if (this->m_who == 我方坦克A)
	{
		...
		{
			if (!this->IsMyTankMeetOther(UP,pMyTank, pEnemyTank, map))
				this->m_core.Y--;
			this->m_dir = UP;
		}
		else if (KEYDOWN('S'))
		{
			if (!this->IsMyTankMeetOther(DOWN, pMyTank,pEnemyTank, map))
				this->m_core.Y++;
			this->m_dir = DOWN;
		}
		else if (KEYDOWN('A'))
		{
			if (!this->IsMyTankMeetOther(LEFT, pMyTank,pEnemyTank, map))
		...
		{
			if (!this->IsMyTankMeetOther(RIGHT, pMyTank,pEnemyTank, map))
...
bool CTank::IsMyTankMeetOther(int dir,CTank * pMyTank, CTank* pEnemyTank, CMap map)
{
	switch (dir)
	{
	case UP:
